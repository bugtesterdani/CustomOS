...
uint8_t parseELFFile(uint16_t *elffile, uint32_t sizeBytes)
{
    ...
    ELF_header_table_t *header_table = ((ELF_header_table_t*)(&(ELF->HeaderTablePosition)));
...
void PreparePages(ELF_header_t *ELF, ELF_header_table_t *header_table, uint32_t *start_address)
{
    ...
    {        
        ...
        memcp(((uint8_t*)((uint32_t)ELF)), (uint8_t*)(block_addr + block_offset), start_offset, size / 4, 0);
        if (i == 0)
        {
            *start_address = (block_addr + block_offset);
        }
        map_page((uint32_t*)(block_addr + block_offset), &save_address, 0x7);
    }
    return 1;
}
...
void Parsing(char *name)
{
    ...
    loader_bytes[11] = 0xE8;
    loader_bytes[12] = 0xF0;
    loader_bytes[13] = 0xFF;
    loader_bytes[14] = 0xFF;
    loader_bytes[15] = 0xFE;
    loader_bytes[16] = 0xC3;
    memcp(loader_bytes, (uint8_t*)0x00, 0, 17, 0);
